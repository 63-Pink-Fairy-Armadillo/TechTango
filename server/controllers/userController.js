...
/*
 * getAllUserInformation - get all information of users from database
 */
userController.getAllUserInformation = async (req, res, next) => {
  try {
    const allUserInformations = await User.find();
    res.locals.allUserInformations = allUserInformations;
    return next();
  } catch (err) {
    return next({
